...
(define (make-rectangle width height colorf)
    (returning (poly (polygon)) (pascal-for (i 0 4 1)
        (let* ((f (lambda (x) (sqrt (* 2 (square x)))))
               ...
               (ang::double (+ (/ tau 8) (* tau (/ i 4))))
            )
            (let-values (((r g b) (colorf i)))
                (invoke poly:verts 'add (returning (v (vertex))
                    (set!* (v:x v:y v:z) ((* xrad (cos ang)) (* yrad (sin ang)) 0))
                    (set!* (v:r v:g v:b) (r g b))
                ))
            )
        )
...
(define (inside-poly? cbuf::concatenated-buffer vertidx::polygon-index ox oy rot px py)
    (let* ((buf cbuf:buf)
           (offset (cbuf:offset vertidx))
...
(define (concatenate-buffers buffers)
    ...
    (returning (cbuf::concatenated-buffer (concatenated-buffer total-capacity))
...
(define (set-polygons-buffer gl2::GL2 cbuf::concatenated-buffer)
    (returning (vbo-ptr-ref::int[] (int[] length: 1))
        (gl2:glGenBuffers 1 vbo-ptr-ref 0)
        (gl2:glBindBuffer gl2:GL_ARRAY_BUFFER (vbo-ptr-ref 0))
        (invoke cbuf:buf 'rewind)
;        (let ((tmp (float[] length: (invoke cbuf:buf 'capacity))))
;            (invoke cbuf:buf 'get tmp)
;            (display tmp) (newline)
;            (display tmp:length) (newline)
;        )
        (invoke cbuf:buf 'rewind)
        ; the multiplication by 4 is because glBufferData takes a size in bytes
        (gl2:glBufferData gl2:GL_ARRAY_BUFFER (* (invoke cbuf:buf 'capacity) 4) cbuf:buf gl2:GL_DYNAMIC_DRAW)
    )
)
...
(define (drawPolygon gl2::GL2 cbuf::concatenated-buffer x y rot vertidx::polygon-index)
    (gl2:glMatrixMode gl2:GL_MODELVIEW)
    (gl2:glPushMatrix)
    (gl2:glTranslated x y 0)
    (gl2:glRotated (rad->deg rot) 0 0 1)
;    (display 'hello) (newline)
;    (display cbuf) (newline)
;    (display (invoke cbuf:starts 'keySet)) (newline)
;    ;(display vertidx:hash) (newline)
;    (display (identity-hash vertidx:buffer)) (newline)
;    (display vertidx:offset) (newline)
;    (display vertidx:numverts) (newline)
    (define offset (cbuf:offset vertidx))
;    (display offset) (newline)
    (gl2:glDrawArrays gl2:GL_POLYGON offset vertidx:numverts)
    (gl2:glPopMatrix)
)

(define (draw-dotted-line gl2::GL2 cbuf::concatenated-buffer dot x1 y1 x2 y2)
    (let* ( (density 20)
            (dx (- x2 x1))
            (dy (- y2 y1))
