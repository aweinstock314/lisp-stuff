...
; this will scale linearly in then number of keys to consider, consider replacing with hashtable despite overhead?
(define *left-held* #f)
(define *right-held* #f)
(define *up-held* #f)
(define *down-held* #f)
(define (handle-keypress code val)
    (if (equal? code KeyEvent:VK_LEFT) (set! *left-held* val))
    (if (equal? code KeyEvent:VK_RIGHT) (set! *right-held* val))
    (if (equal? code KeyEvent:VK_UP) (set! *up-held* val))
    (if (equal? code KeyEvent:VK_DOWN) (set! *down-held* val))
    ;case won't work because it quotes the keys (it also uses eqv?, but that's not a problem here)
    ;maybe write a macro that doesn't do that later?
;    (case code
;        ((KeyEvent:VK_LEFT) (set! *left-held* val))
;        ((KeyEvent:VK_RIGHT) (set! *right-held* val))
;        ((KeyEvent:VK_UP) (set! *up-held* val))
;        ((KeyEvent:VK_DOWN) (set! *down-held* val))
;    )
)
...
(define (handle-movement)
    (if *left-held* (inc! player-ship:rot *rotation-delta*))
    (if *right-held* (inc! player-ship:rot (- *rotation-delta*)))
    (if *up-held* (inc! player-ship:velocity *velocity-delta*))
    (if *down-held* (inc! player-ship:velocity (- *velocity-delta*)))
...
(glcanv:addKeyListener (object (java.awt.event.KeyListener)
    ((keyPressed ev) 
        ;(printf "keypressed %s \n" (ev:getKeyCode))
        (handle-keypress (ev:getKeyCode) #t)
    )
    ((keyReleased ev)
        ;(printf "keyreleased %s \n" (ev:getKeyCode))
        (handle-keypress (ev:getKeyCode) #f)
    )
    ((keyTyped ev) #!void)
))
