...
(define (initialize-label c::java.awt.Container label::JLabel x y)
    (label:setPreferredSize +label-dimensions+)
    (c:add label)
    (label:setBounds x y (+label-dimensions+:getWidth) (+label-dimensions+:getHeight))
)
(define (update-label label::JLabel fmt::String val::String)
...
(define-simple-class shot (drawer)
    ...
    (player::ship) ; originating player, used for scoring
    ((*init* shp::ship ix iy irot ivel)
        (set! player shp)
        (set! x ix)
        (set! y iy)
        (set! rot irot)
...
(define-simple-class ship (drawer)
    ...
    ((shoot! gs::game-state)
        (gs:active-shots:add (shot (this) x y rot velocity))
...
(define-simple-class input-controls ()
    ...
    ((process currently-held-keys::HashSet shp::ship gs::game-state)
        ...
        (if (or (key-held? rotate-left) (key-held? rotate-right) (key-held? accelerate) (key-held? decelerate) (key-held? shoot))
            (invoke gs:active-respawnboxes 'remove shp)
        )
    )
)
...
(define-simple-class game-state ()
    ((*init* is::interface-state)
        ...
        (let ((ship1 (ship interface)))
            ...
            (scorelabels:put 1 (JLabel (String:format +score-fmtstr+ +initial-score+)))
            (liveslabels:put 1 (JLabel (String:format +lives-fmtstr+ +initial-lives+)))
        )
        (if +multiplayer-mode+ (let ((ship2 (ship interface 2 0 0 1)))
            ...
            (scorelabels:put 2 (JLabel (String:format +score-fmtstr+ +initial-score+)))
            (liveslabels:put 2 (JLabel (String:format +lives-fmtstr+ +initial-lives+)))
        ))
    )
    (interface::interface-state #!null)
    (player-controls::HashMap[ship input-controls] (HashMap))
    (all-ships::HashMap[integer ship] (HashMap)) ; maybe change to int->player, and have players encapsulate (ship, labels)?
    (active-ships::ArrayList[ship] (ArrayList))
    (active-shots::ArrayList[shot] (ArrayList))
    (active-asteroids::ArrayList[asteroid] (ArrayList))
    (active-respawnboxes::HashSet[ship] (HashSet))
    (level +initial-level+)
    (scorelabels::HashMap[integer JLabel] (HashMap))
    (liveslabels::HashMap[integer JLabel] (HashMap))
    ...
    (levellabel (JLabel (String:format +level-fmtstr+ +initial-level+)))
...
(define (split-if-nearby-asteroid-overlaps-point! gs::game-state is::interface-state s::shot x y rot vel)
...
(define (event-loop gs::game-state is::interface-state)
    ...
    (java-iterate gs:active-shots (s shot iter)
        (s:updatePosition!)
        (if (or (split-if-nearby-asteroid-overlaps-point! gs is s s:x s:y s:rot s:velocity) (s:expired?))
            (iter:remove)
        )
    )
    (java-iterate gs:active-asteroids (a asteroid)
        (a:updatePosition!)
        (if (> (invoke gs:active-respawnboxes 'size) 0) (set-values! (a:x a:y) (push-outside-respawn-safety-box a:x a:y))) ; replace with iterating over the boxes, once multiplayer is added
        (java-iterate gs:active-ships (shp ship)
    ...
    (java-iterate (invoke gs:all-ships 'keySet) (playnum integer)
        (update-label (invoke gs:scorelabels 'get playnum) +score-fmtstr+ (invoke gs:all-ships 'get playnum):score)
        (update-label (invoke gs:liveslabels 'get playnum) +lives-fmtstr+ (invoke gs:all-ships 'get playnum):lives)
    )
...
;        (display proj-matrix) (newline)
    ...
    (define-constant (draw-foreground gl2 cx cy)
        (if (> (invoke gs:active-respawnboxes 'size) 0) (drawPolygon gl2 is:cbuf 0 0 0 +respawn-box-vertidx+))
...
;;        (inplace! (clamp 0 1) +asteroid-speed-multiplier+)
    ...
    ((*init* w::integer h::integer) #!void
        ...
        (setLayout #!null)
        (initialize-label (this) (invoke gamestate:scorelabels 'get 1) 0 0) ; the "'get 1" is a hack, fix later
        (initialize-label (this) (invoke gamestate:liveslabels 'get 1) (- +screen-width+ (+label-dimensions+:getWidth)) 0)
        (when +multiplayer-mode+
            (initialize-label (this) (invoke gamestate:scorelabels 'get 2) +screen-width+ 0)
            (initialize-label (this) (invoke gamestate:liveslabels 'get 2) (- w (+label-dimensions+:getWidth)) 0)
        )
