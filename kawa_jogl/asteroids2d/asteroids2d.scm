...
(define *game-state* (game-state))
(define *interface-state* (interface-state))

(*game-state*:spawn-asteroids! +initial-asteroids-count+)
...
(define (split-if-nearby-asteroid-overlaps-point! gs::game-state x y rot vel)
    (call-with-current-continuation (lambda (break)
        ...
        ) (asteroids-near-point *game-state*:active-asteroids x y .5))
        (break #f)
    ))
)
...
(define (event-loop gs::game-state)
    ...
    (java-iterate gs:active-shots (s shot iter)
        (s:updatePosition!)
        (if (or (split-if-nearby-asteroid-overlaps-point! gs s:x s:y s:rot s:velocity) (s:expired?))
            (iter:remove)
        )
    )
    (java-iterate gs:active-asteroids (a asteroid)
        ...
        (if (inside-poly? *polygons-buffer* a:vertidx a:x a:y a:rot (field gs 'player-ship):x (field gs 'player-ship):y)
            (gs:player-death!)
        )
    )
    (when (and (invoke gs:active-asteroids 'isEmpty) (not *interface-state*:displayed-victory-message))
...
(define (render gl2::GL2 gs::game-state)
...
;        (display proj-matrix) (newline)
    ...
    (define-constant (draw-foreground gl2 cx cy)
        (if gs:respawn-box-active (drawPolygon gl2 0 0 0 +respawn-box-vertidx+))
        (gs:player-ship:draw gl2)
    ...
    (define-constant (draw-whole-area gl2 x y w h)
        ...
        (if (equal? *interface-state*:whole-area-dims #!null)
            (set! *interface-state*:whole-area-dims (int[] x y w h))
        )
        (if (equal? *interface-state*:whole-area-matrix #!null)
            (set! *interface-state*:whole-area-matrix (getPMVMatrix gl2))
        )
    )
    ...
    (define-constant (draw-debugging-poly gl2 x y w h)
        ...
        (receive (px py) *interface-state*:recent-mouse-obj-coords
...
(define (reset-polygons-buffer! gl2::GL2)
        ...
        (set! *interface-state*:shader-program (make-shader-program gl2 (file-as-string-constant "identityshader.vert") (file-as-string-constant "identityshader.frag")))
        (define pos-attrib ::int (gl2:glGetAttribLocation *interface-state*:shader-program "position"))
        (gl2:glVertexAttribPointer pos-attrib 3 gl2:GL_FLOAT #f 24 0)
        (gl2:glEnableVertexAttribArray pos-attrib)
        (define color-attrib ::int (gl2:glGetAttribLocation *interface-state*:shader-program "color"))
...
;;        (inplace! (clamp 0 1) +asteroid-speed-multiplier+)
        (*interface-state*:currently-held-keys:add (ev:getKeyCode))
    )
    ((keyReleased ev)
    ...
    ((mouseClicked ev)
        (unless (equal? *interface-state*:whole-area-matrix #!null)
            (receive (x y) (mousecoords->objcoords (ev:getX) (ev:getY))
                ...
                (split-if-closest-asteroid-overlaps-point! *game-state* x y 0 0)
            )
        )
    )
    ...
    ((*init* w::integer h::integer) #!void
        (setLayout #!null)
        (initialize-label (this) *game-state*:scorelabel 0 0)
        (initialize-label (this) *game-state*:liveslabel (- w (+label-dimensions+:getWidth)) 0)
        (initialize-label (this) *game-state*:levellabel (- w (+label-dimensions+:getWidth)) (+label-dimensions+:getHeight))
        ...
        (set! (this):glcanv (GLCanvas))
        (for-each (lambda (method) (invoke (this):glcanv method (this)))
...
(with-min-ms-per-iteration 10 (synchronized *game-state*:eventloop-render-mutex (event-loop *game-state*)))
