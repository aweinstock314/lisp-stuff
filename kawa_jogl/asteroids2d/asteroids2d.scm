...
(define-simple-class shot (drawer)
    ...
    (verts)
    ((recalcVerts!) (set! verts (calc-poly x y rot (constantly +shot-size+) 10)))
    ((updatePosition!)
        (set! x (+ x (* velocity (cos rot))))
        (set! y (+ y (* velocity (sin rot))))
        (recalcVerts!)
    )
    ((draw gl2) (drawPolygon gl2 +shot-color+ verts))
)
...
(define-simple-class ship (drawer)
    ...
    ((*init*) #!void)
    (verts)
    ((recalcVerts!) (set! verts (calc-poly x y rot (lambda (i) (if (= i 0) (* 2 size) size)) 3))) ; isosceles triangle
    ((getVerts) verts)
    ((updatePosition!)
...
(define (render gl2::GL2)
    ...
    (draw-frame gl2 player-ship:x player-ship:y)
