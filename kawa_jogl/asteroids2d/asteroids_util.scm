...
(define (inside-line? px py x1 y1 x2 y2)
    (let* ((theta1 (ensure-positive-angle (atan2 (- y2 y1) (- x2 x1))))
        ...
        (printf "(%s, %s)\n(%s, %s)\n(%s, %s)\n" px py x1 y1 x2 y2)
        (printf "theta1 %s; theta2: %s\n" theta1 theta2)
        (returning (val (> (- theta2 theta1) 0))
...
(define (matrixmult glmtrx::FloatBuffer m2::float[])
    (define m1 (float[] length: 16))
    (glmtrx:reset)
    (glmtrx:get m1)
    (display m1) (newline)
    (display m2) (newline)
    ; may want to generalize to arbitrarily-sized matrices
    ;  according to the generated bytecode, with-partial-evaluation is a no-op in this case (compiler already does it)
    (returning (m3
    (with-partial-evaluation (+ *) (float[] length: 4
(+ (* (m2 0) (m1 (+ (* 0 4) 0))) (* (m2 1) (m1 (+ (* 1 4) 0))) (* (m2 2) (m1 (+ (* 2 4) 0))) (* (m2 3) (m1 (+ (* 3 4) 0))))
(+ (* (m2 0) (m1 (+ (* 0 4) 1))) (* (m2 1) (m1 (+ (* 1 4) 1))) (* (m2 2) (m1 (+ (* 2 4) 1))) (* (m2 3) (m1 (+ (* 3 4) 1))))
(+ (* (m2 0) (m1 (+ (* 0 4) 2))) (* (m2 1) (m1 (+ (* 1 4) 2))) (* (m2 2) (m1 (+ (* 2 4) 2))) (* (m2 3) (m1 (+ (* 3 4) 2))))
(+ (* (m2 0) (m1 (+ (* 0 4) 3))) (* (m2 1) (m1 (+ (* 1 4) 3))) (* (m2 2) (m1 (+ (* 2 4) 3))) (* (m2 3) (m1 (+ (* 3 4) 3))))
    ))
    ) (display m3) (newline))
)
...
(define (untranslate/rotate ox oy rot px py)
    ...
    (define m2 (mtrx:glGetMviMatrixf))
    (mtrx:update)
    (matrixmult m2 (float[] px py 0 1))
)
